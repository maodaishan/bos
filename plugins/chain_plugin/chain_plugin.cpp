...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( config::system_account_name, params.account_name, N(personaldata) ));
	  if (t_id != nullptr) {
         const auto &idx = d.get_index<key_value_index, by_scope_primary>();
         ...
         if ( it != idx.end() ) {
            vector<char> data;
            copy_inline_row(*it, data);
            variant raw_data= abis.binary_to_variant( "personal", data, abi_serializer_max_time, shorten_abi_errors );
